...
public class CustomTable extends JTable {
	...
	/*
	 * Parameterized constructor. Accepts a non-null CustomTableModel.
	 */
	public CustomTable(CustomTableModel m) {
		...
		// There are no default renderers and editors for Date, Timestamp, and
		// Time
		// so we need to create them. I will only create the renderer and editor
		// for
	...
	/*
	 * This method picks good column widths. It is a modified version of
	 * initColumnSizes() in TableRenderDemo.java in Sun's Swing tutorial. It
	 * works only with Java 2 SDK versions 1.3 or greater.
	 */
	private void initColumnSizes() {
		...
		for (int i = 0; i < numColumns; i++) {
			...
			comp = getTableHeader().getDefaultRenderer().getTableCellRendererComponent(this, column.getHeaderValue(),
					false, false, 0, i);

			headerWidth = comp.getPreferredSize().width;

			for (int j = 0; j < numRows; j++) {
				comp = getDefaultRenderer(model.getColumnClass(i)).getTableCellRendererComponent(this, getValueAt(j, i),
						false, false, j, i);
	 ...
	 * 
	 * Whenever an exception occurs in CustomTable, an exception event is sent
	 * to all registered ExceptionListeners.
	 ******************************************************************************/
	...
	class CellMouseListener extends MouseMotionAdapter {
		...
		public void mouseMoved(MouseEvent event) {
			...
			if (!table.getSelectionModel().isSelectedIndex(tempRow)
				...
				// Remove the old tool tip from the previous cell selection (if
				// there is one).
				// Don't remove if the mouse pointer is still in the same cell.
				if ((row != -1 && col != -1) && (row != tempRow || col != tempCol)) {
					((JComponent) (table.getCellRenderer(row, col).getTableCellRendererComponent(table, null, false,
							false, row, col))).setToolTipText(null);

					// set row and col to -1 so we don't end up later removing
					// the tool tip again
					row = -1;
					col = -1;
				}
			...
			// return if the mouse pointer is in the same cell and the value in
			// the cell
			// has not changed
			if (row == tempRow && col == tempCol) {
				if ((obj == null && oldValue == null) || (obj != null && obj.equals(oldValue)))
					return;
			}

			// only when tempRow, tempCol, obj represent a valid cell whose
			// value is
			// different from its old value can we set the values for row, col,
			// and oldValue
			row = tempRow;
			col = tempCol;
			oldValue = obj;
			...
			if (obj != null) {
				...
				if (value.length() != 0) {
					((JComponent) (table.getCellRenderer(row, col).getTableCellRendererComponent(table, null, false,
							false, row, col))).setToolTipText(value);
				} else {
					((JComponent) (table.getCellRenderer(row, col).getTableCellRendererComponent(table, null, false,
							false, row, col))).setToolTipText(null);
				}
			} else {
				((JComponent) (table.getCellRenderer(row, col).getTableCellRendererComponent(table, null, false, false,
						row, col))).setToolTipText(null);
			}
		}
	}

	/*
	 * Cell editor for java.sql.Date Limited error checking is performed. Dates
	 * must be in JDBC format.
	 */
	class DateCellEditor extends DefaultCellEditor {
		...
		public Object getCellEditorValue() {
			try {
				...
				// this line will automatically generate an exception in
				// CustomTableModel
