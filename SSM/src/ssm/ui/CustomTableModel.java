...
public class CustomTableModel extends AbstractTableModel {
	...
	/*
	 * Parameterized constructor. Accepts a non-null database connection and a
	 * non-null scrollable ResultSet argument. The data in the ResultSet is
	 * copied to the rows vector. If changes in the table's data are to be
	 * propagated to the database, the result set must be updatable and the data
	 * represented by the result set must be stored in the database represented
	...
	public CustomTableModel(Connection connect, ResultSet rset) {
		try {
			...
			while (rs.next()) {
				Vector<Object> tempRow = new Vector<Object>(numColumns);
	...
	/*
	 * Returns the most specific superclass for all the cell values in the given
	 * column. This method is used by JTable to set up a default renderer and
	 * editor for the column. If you don't like the default renderer and editor,
	 * you can always create your own. Note: in Java 2 SDK version 1.2x, some
	 * column classes don't have a default renderer and editor, e.g., any class
	 * derived from Number.
	 */
	...
	public Object getValueAt(int rowIndex, int columnIndex) {
		Vector<Object> aRow = (Vector<Object>) rows.get(rowIndex);

		return aRow.get(columnIndex);
	}

	/*
	 * This method determines whether a setValueAt() on the given cell will
	 * change its value
	 * 
	 */
	...
	/*
	 * Sets value at the given cell to aValue. If the ResultSet is updatable
	 * then the ResultSet and database is updated.
	 */
	 ...
	 * 
	 * Whenever an exception occurs in CustomTableModel, an exception event is
	 * sent to all registered ExceptionListeners.
	 ******************************************************************************/
	 ...
	 * similar to the sample in the Java 2 API documentation for the
	...
	/*
	 * This class sorts the data in a given column. This class will not sort if
	 * the result set is updatable because after the table is sorted, a cell's
	 * row index in the table won't match the cell's row index in the result
	 * set. This means that updates to the table won't be properly propagated to
	 * the database.
	 */
	class Sorter extends MouseAdapter implements Comparator<Object> {
		JTable table = null;
		...
		/*
		 * Registers the current object to receive mouse events from the table
		 * header
		 */
		...
		/*
		 * When the user double clicks the header, the column will be sorted in
		 * ascending order. If the shift key was held down as well then the
		...
		public void mouseClicked(MouseEvent e) {
			...
			if (e.getClickCount() == 2) {
				...
				if (sortIndex >= 0) {
					...
					// getColumn() requires an index in the view's coordinate
					// system,
		...
		/**********************************************
		 * Begin Comparator interface implementation. By implementing this
		 * interface, we can sort the objects in the table.
		 **********************************************/

		/*
		 * This compares the column value of one row to the value of another.
		 */
		@SuppressWarnings("unchecked")
		public int compare(Object obj1, Object obj2) {
			int result = -99;

			Object o1 = ((Vector<Object>) obj1).get(sortIndex);
			Object o2 = ((Vector<Object>) obj2).get(sortIndex);
