...
public class Hex
{
	...
	public static String toStringSpace(byte[] ba, int offset, int length) 
	{
    ...
    public static String toStringSpace(byte[] ba) 
	{
        return toStringSpace(ba, 0, ba.length);
    }
	...
	/*
	** Method name:	String2Byte
	** Function:	return a byte array from a string 
	** Input:		String input
	** output:		byte[]
	** Author:		Fan Xuesong
	** Create Date: 2001.11.29
	*/
	...
	/*
	** Method name:	String2Byte
	** Function:	return a byte array from a string in length "len".
	** Input:	String  input		//source string
	**		int	len		//the length of return byte[].
	**		byte	mend		//if input String not long enought, what char will be ment.
	**		String	position	//the ment char's position( fore/rear)
	** output:		byte[]
	** Author:		Fan Xuesong
	** Create Date: 2001.11.29
	*/
	public static byte[] String2Byte( String input, int len, byte mend, String position )
	{
		byte[] output = new byte[len];
		int lenInput, i, j;

		lenInput = input.length();
		if (position.compareTo("fore")==0)
		{
			for(i=0; i<len-lenInput; i++)
			{
				output[i] = mend;
			}
			for(j=i; j<len; j++)
			{
				output[j] = (byte)(input.charAt(j-i));
			}

		}
		else
//		if (position.compareTo("rear")==0)	// if not in the fore then append at the rear.
		{
			for(i=0; i<input.length(); i++)
			{
				output[i] = (byte)(input.charAt(i));
	...
	/*
	** Method name:	String82Byte
	** Function:	return a byte array from a string 
	** Input:		String input: the string length must be less the 8
	** output:		byte[]	: if string length<8 then append '0'
	** Author:		Fan Xuesong
	** Create Date: 2001.11.29
	*/
	public static byte[] String82Byte( String input )
	{
